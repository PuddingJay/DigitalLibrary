...
const DetailBuku = ({match}) => {
  ...
  useEffect(() => {
    const fetchCatalogItem = async () => {
      try {
        const response = await axios.get(`http://localhost:3005/BookRoute/book/${match.params.kode_buku}`);
              ...
              <Col className="mb-8">
                <Row>
                  <div className="product">
                    <img src={catalogItem.cover_buku} alt={catalogItem.judul} style={{ width: "20rem", height: "25rem" }} />
                    <Row md={1}>
                      <br></br>
                      <br></br>
